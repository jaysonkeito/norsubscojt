...
<p class="text-muted mb-4">Welcome, {{ explode(' ', $user->name)[0] }}! Fill in your details — everything is saved in one step. You won't be able to use the rest of the system until this is submitted and approved.</p>
...
<div class="card p-4" style="max-width:860px;">
    <form method="POST" action="{{ route('account-completion.store') }}" enctype="multipart/form-data" autocomplete="off">
        ...
        <h6 class="text-uppercase text-muted small fw-bold mb-3" style="letter-spacing:0.05em;">Information</h6>
        <div class="row">
        ...
        <div class="row">
            ...
            <div class="col-md-6 mb-3">
                ...
                <select name="designation" id="designationTitle" class="form-select" required onchange="syncRequiredFields()">
                    <option value="">Select designation</option>
                    @foreach($designationOptions as $title)
                        <option value="{{ $title }}" {{ old('designation') === $title ? 'selected' : '' }}>{{ $title }}</option>
                    @endforeach
                </select>
                <small class="text-muted" id="designationHint">Your designation determines which fields are required and who approves your account.</small>
            </div>
        </div>
        <div class="row">
            <div class="col-md-6 mb-3">
        ...
        <div class="row">
            ...
            <div class="col-md-4 mb-3">
                ...
                <input type="text" name="qualification" class="form-control" placeholder="e.g. MA in Education" value="{{ old('qualification') }}">
            </div>
            <div class="col-md-4 mb-3">
        ...
        <div id="companyFields">
            <h6 class="text-uppercase text-muted small fw-bold mb-3" style="letter-spacing:0.05em;">Office / Company Details <span class="text-normal text-muted fw-normal">(for Office / Company designation)</span></h6>
            <div class="row">
                <div class="col-md-6 mb-3">
...
<script>
// The Designation field (right after Department) decides the role:
// 'Dean' → dean, 'Office / Company' → company, anything else → coordinator.
// All sections stay visible; it only controls which fields are REQUIRED.
function currentRole() {
    const title = document.getElementById('designationTitle').value;
    if (!title) return '';
    if (title === 'Dean') return 'dean';
    if (title === 'Office / Company') return 'company';
    return 'coordinator';
}
...
function syncRequiredFields() {
    ...
    document.querySelectorAll('[data-req]').forEach(el => {
        el.required = el.dataset.req === role;
    });
    ...
    const hints = {
        ...
        company: 'Fill in the Office / Company Details section below. Office/Company accounts are approved by the Dean.',
    };
    document.getElementById('designationHint').textContent = hints[role]
        || 'Your designation determines which fields are required and who approves your account.';
}
